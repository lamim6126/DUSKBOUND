...
static const B2Slide preSlides[] = {
    { "THE SANCTUM SORCERER",
      "Master of the arcane arts...",
      "Her spells tear through flesh and bone...",
      "None have survived her sanctum.",
      40, 10, 80, 300 },

    { "PREPARE FOR BATTLE",
      "Magic is her blade...",
      "The Sorcerer watches from within...",
      "Press SPACE to fight!",
      30, 5, 60, 300 }
};

static const B2Slide postSlides[] = {
    { "THE SANCTUM IS SILENT",
      "The last spell fades to dust...",
      "Its keeper is defeated...",
      "Dark magic no longer guards the halls.",
      35, 10, 70, 260 },

    { "VICTORY",
      "The sanctum now belongs to the living.",
      "Return to the world map.",
      "One final evil still remains.",
      25, 5, 55, 260 }
};

BossCutscene2::BossCutscene2(Mode mode)
    : slideIndex(0), timer(0), fadeTimer(0), mode(mode) {
}

void BossCutscene2::update() {
    const B2Slide* slides = (mode == PRE_BOSS) ? preSlides : postSlides;
    ...
    if (slideIndex >= slideCount) {
        if (mode == PRE_BOSS) {
            Engine::changeScene(new BossFightScene2());
        } else {
            // Show the next destination as soon as the player returns to the world.
            setWorldBanner("THRONE ROOM LOCATION REVEALED", 180);
            Engine::changeScene(new PlayScene());
        }
    }
}

void BossCutscene2::draw() {
    const B2Slide* slides = (mode == PRE_BOSS) ? preSlides : postSlides;
    const int slideCount = 2;
    if (slideIndex >= slideCount) return;

    const B2Slide& s = slides[slideIndex];
    ...
    drawTextC(tx, 360, s.title, 200,100,255);
    ...
    for (int i = 0; i < slideCount; i++) {
        if (i == slideIndex) { setColor(200,100,255); fillRect(sx + i * 20 - 4, 155, 9, 9); }
